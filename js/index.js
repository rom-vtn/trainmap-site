...
function displayPage(pageNumber) {
    ...
    for (const inSliceIndex in sightSlice) {
        ...
        const line = `<div class="row py-2 border" id="sight-row-${(+PAGE_LENGTH)*(+pageNumber) + (+inSliceIndex)}">
            <div class="col-12 col-md-12 text-center p-2" style="${getRouteColorCss(realTrainSight.sight.trip.route)};text-align:auto">
                <h2>${realTrainSight.sight.route_name} → ${realTrainSight.sight.last_st.stop.stop_name}</h2>
                ${realTrainSight.sight.trip.short_name===""?"":"<p>("+realTrainSight.sight.trip.short_name+")</p>"}</div>
